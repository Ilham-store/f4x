...
<body class="bg-gray-100 py-10 print:bg-white print:py-0">
    ...
    <script>
        async function generatePDF() {
            ...
            try {
                // 3. PENYESUAIAN LAYOUT (Agar bisa di-capture utuh meski panjang)
                element.style.width = '794px';
                element.style.maxWidth = '794px';
                element.style.margin = '0';
                element.style.boxShadow = 'none';
                element.style.overflow = 'visible'; // Pastikan konten bawah tidak tersembunyi

                // Kasih jeda sedikit agar browser (terutama mobile) sempat re-layout
                await new Promise(resolve => setTimeout(resolve, 250));

                // 4. GENERATE GAMBAR (Hanya 1x capture panjang)
                const dataUrl = await htmlToImage.toPng(element, {
                    backgroundColor: '#ffffff',
                    pixelRatio: 1.5, // 1.5 adalah angka 'sweet spot' untuk HP agar jernih tapi tidak crash
                    skipFonts: true,
                ...
                // 5. LOGIKA MULTI-PAGE (Kunci agar tidak terpotong)
                ...
                // Tambahkan halaman baru jika masih ada sisa tinggi gambar
                while (heightLeft > 0) {
                ...
                alert('Gagal membuat PDF. Coba gunakan browser Chrome atau Safari.');
            } finally {
